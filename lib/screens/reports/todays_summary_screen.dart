...
class _TodaySummaryTabState extends State<TodaySummaryTab> {
  ...
  double _getCategoryQty(_CategorySummaryData cat) {
    switch (_selectedFlow) {
      case TransactionFlowFilter.inward:
        return cat.totalInward;
      case TransactionFlowFilter.outward:
        return cat.totalOutward;
      case TransactionFlowFilter.net:
        return cat.totalNet;
    }
  }

  double _getSizeQty(_TodaySizeRowData size) {
    switch (_selectedFlow) {
      case TransactionFlowFilter.inward:
        return size.inward;
      case TransactionFlowFilter.outward:
  ...
  (Color bg, Color text) _getFlowBadgeColors(double qty) {
    switch (_selectedFlow) {
      case TransactionFlowFilter.inward:
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // MOBILE (< 1024px) CARD-BASED SUMMARY VIEW
  // ───────────────────────────────────────────────────────────────────────────
  Widget _buildMobileSummaryTableView(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final cat = categories[index];
        final double qty = _getCategoryQty(cat);
        final (pillBgColor, pillTextColor) = _getFlowBadgeColors(qty);
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // MOBILE (< 1024px) DETAILED ACCORDION VIEW
  // ───────────────────────────────────────────────────────────────────────────
  Widget _buildMobileDetailedAccordionView(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final double qty = _getCategoryQty(categoryData);
          ...
          child: Column(
            ...
            children: [
              ...
              if (displaySizes.isNotEmpty) ...[
                for (int sIdx = 0; sIdx < displaySizes.length; sIdx++) ...[
                  const Divider(
                    height: 1,
                    thickness: 1,
                    color: Color(0xFFF1F5F9),
                  ...
                  Builder(
                    builder: (context) {
                      final size = displaySizes[sIdx];
                      final double sizeQty = _getSizeQty(size);
                      final Color sizeColor;
                      switch (_selectedFlow) {
                        case TransactionFlowFilter.inward:
                          sizeColor = const Color(0xFF16A34A);
                          break;
                        case TransactionFlowFilter.outward:
                          sizeColor = const Color(0xFFDC2626);
                          break;
                        case TransactionFlowFilter.net:
                          sizeColor = sizeQty < -0.0001
                              ? const Color(0xFFDC2626)
                      ...
                      final String formattedLabel =
                          EnterpriseStockMovementTable.appendKgSuffix(
                              categoryData.name, size.sizeLabel);
